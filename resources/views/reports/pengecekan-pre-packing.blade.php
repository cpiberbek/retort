...
@php
    $dateFilter = request('date') ? \Carbon\Carbon::parse(request('date'))->format('d-m-Y') : 'Semua Tanggal';
@endphp

<table width="100%" class="tbl-header" style="font-size:11pt !important;">
    <tr>
        <td width="15%">Hari / Tanggal: {{ $dateFilter }}</td>
        <td width="85%"></td>
    </tr>
</table>
...
<table width="100%" class="tbl-main small" cellpadding="3" align="center">
    <tr style="font-weight: bold;">
        ...
        <th rowspan="2" style="width:7%;">No.<br>Conveyor</th>
        <th rowspan="2" style="width:8%;">Suhu<br>Varian</th>
        <th rowspan="2" style="width:10%;">Bagian Badan Sosis</th>
        <th colspan="2" style="width:12%;">Air (%)</th>
        <th colspan="2" style="width:12%;">Minyak (%)</th>
        <th colspan="2" style="width:18%;">Berat Varian per</th>
        <th rowspan="2" style="width:10%;">PARAF<br>QC</th>
    </tr>
    <tr style="font-weight: bold;">
...
@php
    $catatan = $prepackings
        ->map(function ($item) {
            $kode = \App\Models\Mincing::where('uuid', $item->kode_produksi)->value('kode_produksi') ?? '-';
