...
@section('content')
    <div class="container-fluid py-0">
            ...
            <div class="btn-group" role="group">
                ...
                @can('can access export')
                    @if (request('date') && request('shift'))
                        <a href="{{ route('loading-produks.exportPdf', ['date' => request('date'), 'shift' => request('shift')]) }}"
        ...
        <form id="filterForm" method="GET" action="{{ route('loading-produks.index') }}"
                            ...
                            @forelse ($produks as $produk)
                                <tr>
                                    ...
                                    <td class="align-middle">
                                        <button type="button" class="btn btn-primary btn-sm btn-kode-produksi"
                                            data-url="{{ route('loading-produks.kode-produksi', $produk->uuid) }}"
                                            data-bs-toggle="modal" data-bs-target="#modalEkspedisi">
                                            Lihat Kode Produksi
                                        </button>
                                    </td>
                                        ...
                                        <div class="d-flex justify-content-center align-items-center">
                                            ...
                                            @can('can access verification button')
                                                <button type="button" class="btn btn-primary btn-sm fw-bold shadow-sm mx-1"
                                                    data-bs-toggle="modal" data-bs-target="#verifyModal{{ $produk->uuid }}">
                                                    <i class="bi bi-shield-check me-1"></i> Verifikasi
                                                </button>
                                            @endcan

                                            {{-- 1. Detail --}}
                                            <a href="{{ route('loading-produks.show', $produk->uuid) }}"
                                                class="btn btn-outline-primary btn-sm mx-1" title="Detail">
                                                <i class="bi bi-eye"></i>
                                            </a>

                                            {{-- 2. Edit --}}
                                            @can('can access edit button')
                                                <a href="{{ route('loading-produks.edit', $produk->uuid) }}"
                                            ...
                                            @can('can access update button')
                                                <a href="{{ route('loading-produks.edit-details', $produk->uuid) }}"
                                                    class="btn btn-info btn-sm mx-1 text-white" title="Lengkapi Data">
                                            ...
                                            @can('can access delete button')
                                                <form action="{{ route('loading-produks.destroy', $produk->uuid) }}"
    ...
    <div class="modal fade" id="modalEkspedisi" tabindex="-1" aria-labelledby="modalEkspedisiLabel"
        aria-hidden="true">
    ...
    <div class="modal fade" id="warningModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Silakan pilih <b>Tanggal</b> dan <b>Shift</b> yang spesifik di bagian filter terlebih dahulu sebelum
                    melakukan export.
                </div>
...
--}}
    @foreach ($produks as $produk)
        <div class="modal fade modal-verification" id="verifyModal{{ $produk->uuid }}" tabindex="-1"
            aria-labelledby="verifyModalLabel{{ $produk->uuid }}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <form action="{{ route('loading-produks.verify', $produk->uuid) }}" method="POST">
                        ...
                        <div class="modal-header">
                            <h5 class="modal-title fw-bolder fs-4 text-uppercase"
                                id="verifyModalLabel{{ $produk->uuid }}"><i class="bi bi-gear-fill me-2"></i>Verification
                            </h5>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-4">
                                <label for="status_spv_{{ $produk->uuid }}"
                                    class="form-label fw-bold d-block text-center mb-2">Pilih Status Verifikasi</label>
                                <select name="status_spv" id="status_spv_{{ $produk->uuid }}"
                                    ...
                                    <option value="1" {{ old('status_spv', $currentStatus) != 2 ? 'selected' : '' }}
                                        style="color: #198754;">✅ Verified (Disetujui)</option>
                                    <option value="2" {{ old('status_spv', $currentStatus) == 2 ? 'selected' : '' }}
                                        style="color: #dc3545;">❌ Revision (Perlu Perbaikan)</option>
                                </select>
                            </div>

                            <div class="mb-3">
                                <label for="catatan_spv_{{ $produk->uuid }}" class="form-label fw-bold">Catatan
                                    Verifikasi (Wajib jika 'Revision')</label>
                                <textarea name="catatan_spv" id="catatan_spv_{{ $produk->uuid }}" rows="4"
                                    class="form-control shadow-none textarea-custom" placeholder="Tuliskan catatan perbaikan...">{{ old('catatan_spv', $produk->catatan_spv ?? '') }}</textarea>
                            </div>
                        </div>
