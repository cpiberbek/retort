...
@section('content')
    <div class="container-fluid py-0">
        ...
        <div class="d-sm-flex justify-content-between align-items-center mb-4">
            ...
            <div class="btn-group" role="group">
                ...
                @can('can access export')
                    <a href="{{ route('packaging-inspections.exportPdf', ['start_date' => request('start_date'), 'shift' => request('shift')]) }}"
        ...
        {{-- 2. FILTER SECTION --}}
        <form id="filterForm" method="GET" action="{{ route('packaging-inspections.index') }}"
                                        ...
                                        <div class="d-flex justify-content-center align-items-center">
                                            ...
                                            @can('can access verification button')
                                                <button type="button" class="btn btn-primary btn-sm fw-bold shadow-sm mx-1"
                                                    ...
                                                    data-bs-target="#verifyModal{{ $inspection->uuid }}">
                                                    <i class="bi bi-shield-check me-1"></i> Verifikasi
                                                </button>
                                            @endcan

                                            {{-- 2. Detail --}}
                                            <a href="{{ route('packaging-inspections.show', $inspection) }}"
                                                class="btn btn-outline-primary btn-sm mx-1" title="Lihat Detail">
                                                <i class="bi bi-eye"></i>
                                            </a>

                                            {{-- 3. Edit --}}
                                            @can('can access edit button')
                                                <a href="{{ route('packaging-inspections.edit', $inspection) }}"
                                                    class="btn btn-warning btn-sm mx-1" title="Edit Data">
                                            ...
                                            @can('can access update button')
                                                <a href="{{ route('packaging-inspections.edit-for-update', $inspection) }}"
                                            ...
                                            @can('can access delete button')
                                                <form action="{{ route('packaging-inspections.destroy', $inspection) }}"
...
--}}
    @foreach ($inspections as $inspection)
        <div class="modal fade modal-verification" id="verifyModal{{ $inspection->uuid }}" tabindex="-1"
            aria-labelledby="verifyModalLabel{{ $inspection->uuid }}" aria-hidden="true">
                    ...
                    <form action="{{ route('packaging-inspections.verify', $inspection->uuid) }}" method="POST">
                        ...
                        <div class="modal-header">
                            <h5 class="modal-title fw-bolder fs-4 text-uppercase"
                                id="verifyModalLabel{{ $inspection->uuid }}"><i
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label for="catatan_spv_{{ $inspection->uuid }}" class="form-label fw-bold">Catatan
                                    Verifikasi (Wajib jika 'Revision')</label>
                                <textarea name="catatan_spv" id="catatan_spv_{{ $inspection->uuid }}" rows="4"
                                    class="form-control shadow-none textarea-custom" placeholder="Tuliskan catatan perbaikan...">{{ old('catatan_spv', $inspection->catatan_spv) }}</textarea>
                            </div>
                        </div>
