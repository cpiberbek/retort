...
@section('content')
    <div class="container-fluid py-0">
        ...
        <form id="filterForm" method="GET" action="{{ route('pemeriksaan_retain.index') }}"
                            ...
                            @forelse ($pemeriksaanRetains as $p)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        {{ $loop->iteration + ($pemeriksaanRetains->currentPage() - 1) * $pemeriksaanRetains->perPage() }}
                                    </td>
                                        ...
                                        <div class="d-flex justify-content-center align-items-center">
                                            ...
                                            @can('can access verification button')
                                                <button type="button" class="btn btn-primary btn-sm fw-bold shadow-sm mx-1"
                                                    data-bs-toggle="modal" data-bs-target="#verifyModal{{ $p->uuid }}">
                                                    <i class="bi bi-shield-check me-1"></i> Verifikasi
                                                </button>
                                            @endcan

                                            {{-- 1. Detail --}}
                                            <a href="{{ route('pemeriksaan_retain.show', $p->uuid) }}"
                                            ...
                                            @can('can access edit button')
                                                <a href="{{ route('pemeriksaan_retain.edit', $p->uuid) }}"
                                            ...
                                            @can('can access update button')
                                                <a href="{{ route('pemeriksaan_retain.edit-for-update', $p->uuid) }}"
                                                    class="btn btn-info btn-sm mx-1 text-white" title="Update Form">
                                            ...
                                            @can('can access delete button')
                                                <form action="{{ route('pemeriksaan_retain.destroy', $p->uuid) }}"
                                                    method="POST" class="d-inline"
                                                    onsubmit="return confirm('Yakin ingin menghapus data pemeriksaan retain ini?')">
                                                    @csrf
                                                    @method('DELETE')
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    Silakan pilih <b>Tanggal & Kode Batch</b> yang spesifik di bagian filter terlebih dahulu sebelum
                    melakukan export.
                </div>
...
--}}
    @foreach ($pemeriksaanRetains as $p)
        <div class="modal fade modal-verification" id="verifyModal{{ $p->uuid }}" tabindex="-1"
            aria-labelledby="verifyModalLabel{{ $p->uuid }}" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <form action="{{ route('pemeriksaan_retain.verify', $p->uuid) }}" method="POST">
                        ...
                        <div class="modal-header">
                            <h5 class="modal-title fw-bolder fs-4 text-uppercase"
                                id="verifyModalLabel{{ $p->uuid }}"><i class="bi bi-gear-fill me-2"></i>Verification
                            </h5>
                        ...
                        <div class="modal-body">
                            <p class="text-light mb-4">
                                Verifikasi Data Tanggal:
                                <strong>{{ \Carbon\Carbon::parse($p->tanggal)->format('d-m-Y') }}</strong><br>
                                Keterangan: {{ $p->keterangan }}
                            </p>

                            <div class="mb-4">
                                <label for="status_spv_{{ $p->uuid }}"
                                    class="form-label fw-bold d-block text-center mb-2">Pilih Status Verifikasi</label>
                                <select name="status_spv" id="status_spv_{{ $p->uuid }}"
                            ...
                            <div class="mb-3">
                                <label for="catatan_spv_{{ $p->uuid }}" class="form-label fw-bold">Catatan
                                    Verifikasi (Wajib jika 'Revision')</label>
                                <textarea name="catatan_spv" id="catatan_spv_{{ $p->uuid }}" rows="4"
                                    class="form-control shadow-none textarea-custom" placeholder="Tuliskan catatan perbaikan...">{{ old('catatan_spv', $p->catatan_spv ?? '') }}</textarea>
                            </div>
                        </div>
