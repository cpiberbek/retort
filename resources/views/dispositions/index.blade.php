...
@section('content')
    <div class="container-fluid py-0">
        ...
        <div class="d-sm-flex justify-content-between align-items-center mb-4">
            ...
            <div class="btn-group" role="group">
                ...
                @can('can access export')
                    <a href="{{ route('dispositions.exportPdf', ['date' => request('date')]) }}" target="_blank"
        ...
        <form id="filterForm" method="GET" action="{{ route('dispositions.index') }}"
                        ...
                        <tbody>
                            @forelse ($dispositions as $disp)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if ($disp->disposisi_produk)
                                            <span class="badge bg-info">Produk</span>
                                        @endif
                                        @if ($disp->disposisi_material)
                                            <span class="badge bg-warning text-dark">Material</span>
                                        @endif
                                        @if ($disp->disposisi_prosedur)
                                            <span class="badge bg-info text-dark">Prosedur</span>
                                        @endif
                                    </td>
                                        ...
                                        <div class="d-flex justify-content-center align-items-center">
                                            ...
                                            @can('can access verification button')
                                                ...
                                                <button type="button" class="btn btn-primary btn-sm fw-bold shadow-sm mx-1"
                                                    data-bs-toggle="modal" data-bs-target="#verifyModal{{ $disp->uuid }}"
                                            ...
                                            @can('can access detail button')
                                                {{-- 1. Detail --}}
                                                <a href="{{ route('dispositions.show', $disp->uuid) }}"
                                            ...
                                            @can('can access edit button')
                                                {{-- 2. Edit --}}
                                                <a href="{{ route('dispositions.edit', $disp->uuid) }}"
                                            ...
                                            @can('can access delete button')
                                                {{-- 4. Hapus --}}
                                                <form action="{{ route('dispositions.destroy', $disp->uuid) }}"
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center py-5 text-muted">
                                        <div class="mb-2"><i
                                                class="bi bi-clipboard-x display-4 text-secondary opacity-50"></i></div>
...
--}}
    @foreach ($dispositions as $disp)
        <div class="modal fade modal-verification" id="verifyModal{{ $disp->uuid }}" tabindex="-1"
            aria-labelledby="verifyModalLabel{{ $disp->uuid }}" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <form action="{{ route('dispositions.verify', $disp->uuid) }}" method="POST">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                <label for="catatan_spv_{{ $disp->uuid }}" class="form-label fw-bold">Catatan
                                    Verifikasi (Wajib jika 'Revision')</label>
                                <textarea name="catatan_spv" id="catatan_spv_{{ $disp->uuid }}" rows="4"
                                    class="form-control shadow-none textarea-custom" placeholder="Tuliskan catatan perbaikan...">{{ old('catatan_spv', $disp->catatan_spv ?? '') }}</textarea>
                            </div>
                        </div>
                        <div class="modal-footer justify-content-end">
                            ...
                            <button type="submit" class="btn rounded-pill px-5 fw-bolder"
                                style="background-color: var(--accent-color-1); color: var(--text-on-light);">
