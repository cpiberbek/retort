                    ...
                    if (is_array($val)) {
                        return collect($val)
                            ->map(
                                fn(
                                    $v,
                                ) => "<span class='badge bg-gradient bg-light text-dark border border-secondary me-1 mb-1'>$v</span>",
                            )
                            ->implode('');
                    }
                    return e($val ?? '-');
        ...
        <form id="filterForm" method="GET" action="{{ route('pemasakan.index') }}"
            ...
            <div class="row w-100 g-3">
                ...
                <div class="col-md-2 align-self-end">
                    <a href="{{ route('pemasakan.index') }}" class="btn btn-primary mb-2"><i
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                if (exportPdfBtn) {
                    exportPdfBtn.addEventListener('click', function() {
                        ...
                        const exportUrl = "{{ route('pemasakan.exportPdf') }}?" + new URLSearchParams(formData)
                            .toString();
                ...
                if (exportExcelBtn) {
                    exportExcelBtn.addEventListener('click', function() {
                        ...
                        const formData = new FormData(form);
                        const exportUrl = "{{ route('pemasakan.exportExcel') }}?" + new URLSearchParams(
                            formData).toString();
                        window.open(exportUrl, '_blank');
                    });
                }
                ...
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            @forelse ($data as $dep)
                                @php
                                    // FIX: Tidak perlu json_decode karena $casts di model sudah handle menjadi array
                                    $cooking = $dep->cooking ?? [];
                                @endphp
                                ...
                                <tr>
                                    ...
                                    <td class="text-center align-middle">
                                        {{ \Carbon\Carbon::parse($dep->date)->format('d-m-Y') }} | {{ $dep->shift }}
                                    </td>
                                    ...
                                    <td class="text-center align-middle">
                                        @php
                                            $rejects = is_array($dep->total_reject)
                                                ? array_filter($dep->total_reject, fn($v) => !is_null($v) && $v !== '')
                                                : [];
                                        @endphp
                                        ...
                                        @if (!empty($cooking))
                                            ...
                                            <div class="modal fade" id="cookingModal{{ $dep->uuid }}" tabindex="-1"
                                                ...
                                                <div class="modal-dialog modal-xl modal-dialog-scrollable">
                                                    <div
                                                        class="modal-content border-0 shadow-lg rounded-3 overflow-hidden">
                                                        <div class="modal-header bg-primary bg-gradient text-white">
                                                            <h5 class="modal-title fw-semibold"><i
                                                                    class="bi bi-fire me-2"></i> Detail Proses Pemasakan
                                                            </h5>
                                                        ...
                                                        <div class="modal-body p-4 bg-light-subtle">
                                                            @php
                                                                $sections = [
                                                                    ...
                                                                    '5. Sterilisasi' => [
                                                                        'Suhu Air Sterilisasi' =>
                                                                            'suhu_air_sterilisasi',
                                                            ...
                                                            @foreach ($sections as $title => $rows)
                                                                <div class="mb-3">
                                                                    <h6 class="fw-bold text-primary">{{ $title }}
                                                                    </h6>
                                                                        ...
                                                                        <table
                                                                            ...
                                                                            <tbody>
                                                                                @foreach ($rows as $label => $key)
                                                                                    <tr>
                                                                                        <td
                                                                                            class="fw-semibold text-start ps-3 w-50">
                                                                                            {{ $label }}</td>
