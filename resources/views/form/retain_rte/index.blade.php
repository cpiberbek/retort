...
@section('content')
    <div class="container-fluid py-0">
        ...
        <form id="filterForm" method="GET" action="{{ route('retain_rte.index') }}"
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                if (exportBtn) {
                    exportBtn.addEventListener('click', () => {
                        ...
                        const exportUrl = "{{ route('retain_rte.exportPdf') }}?" + new URLSearchParams(formData)
                            .toString();
                        ...
                        <tbody>
                            ...
                            @forelse ($data as $dep)
                                <tr>
                                    ...
                                    <td class="text-center align-middle">
                                        @php
                                            $analisa = is_string($dep->analisa)
                                                ? json_decode($dep->analisa, true)
                                                : $dep->analisa ?? [];
                                            if (!$analisa) {
                                                $analisa = [];
                                            }
                                        ...
                                        @if (!empty($analisa))
                                            <a href="#" data-bs-toggle="modal"
                                                data-bs-target="#analisaModal{{ $dep->uuid }}"
                                                style="font-weight: bold; text-decoration: underline;">
                                                Lihat Analisa
                                            </a>

                                            <div class="modal fade" id="analisaModal{{ $dep->uuid }}" tabindex="-1"
                                                aria-hidden="true">
                                                                                ...
                                                                                <td>
                                                                                    @if (isset($item['release']))
                                                                                        @if ($item['release'] === 'Release')
                                                                                            <span
                                                                                                class="fw-bold text-success">{{ $item['release'] }}</span>
                                                                                        @elseif($item['release'] === 'Tidak Release')
                                                                                            <span
                                                                                                class="fw-bold text-danger">{{ $item['release'] }}</span>
                                                                                        @else
                                                                                            {{ $item['release'] }}
                                                                                        @endif
                                                                                    @else
                                    ...
                                    <td class="text-center align-middle">
                                        ...
                                        @can('can access edit button')
                                            <a href="{{ route('retain_rte.edit.form', $dep->uuid) }}"
                                                class="btn btn-warning btn-sm me-1">
                                                <i class="bi bi-pencil-square"></i> Edit
                                            </a>
                                        @endcan
                                        @can('can access update button')
                                            <a href="{{ route('retain_rte.update.form', $dep->uuid) }}"
                                                class="btn btn-info btn-sm me-1">
                                                <i class="bi bi-pencil"></i> Update
                                            </a>
                                        @endcan
                                        @can('can access delete button')
                                            <form action="{{ route('retain_rte.destroy', $dep->uuid) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        <div class="modal fade" id="verifyModal{{ $dep->uuid }}" tabindex="-1"
                                            ...
                                            <div class="modal-dialog modal-dialog-centered modal-md">
                                                <form action="{{ route('retain_rte.verification.update', $dep->uuid) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
