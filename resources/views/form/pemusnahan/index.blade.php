...
@section('content')
    <div class="container-fluid py-0">
        ...
        <form id="filterForm" method="GET" action="{{ route('pemusnahan.index') }}"
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                if (exportPdfBtn) {
                    exportPdfBtn.addEventListener('click', function() {
                        ...
                        const exportUrl = "{{ route('pemusnahan.exportPdf') }}?" + new URLSearchParams(formData)
                            .toString();
                                ...
                                <tr>
                                    ...
                                    <td class="text-center align-middle">
                                        ...
                                        @can('can access edit button')
                                            <a href="{{ route('pemusnahan.edit.form', $dep->uuid) }}"
                                                class="btn btn-warning btn-sm me-1">
                                                <i class="bi bi-pencil-square"></i> Edit
                                            </a>
                                        @endcan
                                        @can('can access update button')
                                            <a href="{{ route('pemusnahan.update.form', $dep->uuid) }}"
                                                class="btn btn-info btn-sm me-1">
                                                <i class="bi bi-pencil"></i> Update
                                            </a>
                                        @endcan
                                        @can('can access delete button')
                                            <form action="{{ route('pemusnahan.destroy', $dep->uuid) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        <div class="modal fade" id="verifyModal{{ $dep->uuid }}" tabindex="-1"
                                            ...
                                            <div class="modal-dialog modal-dialog-centered modal-md">
                                                <form action="{{ route('pemusnahan.verification.update', $dep->uuid) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
