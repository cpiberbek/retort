...
@section('content')
    <div class="container-fluid py-0">
        ...
        <form id="filterForm" method="GET" action="{{ route('sampling_fg.index') }}"
            ...
            <div class="row w-100">
                ...
                <div class="col-md-3 align-self-end">
                    ...
                    <a href="{{ route('sampling_fg.index') }}" class="btn btn-primary mb-2"><i
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                if (exportPdfBtn) {
                    exportPdfBtn.addEventListener('click', function() {
                        ...
                        const exportUrl = "{{ route('sampling_fg.exportPdf') }}?" + new URLSearchParams(
                            formData).toString();
                        ...
                        <tbody>
                            ...
                            @forelse ($data as $dep)
                                <tr>
                                    ...
                                    <td class="text-center">{{ \Carbon\Carbon::parse($dep->date)->format('d-m-Y') }} |
                                        {{ $dep->shift }}</td>
                                    <td class="text-center">{{ $dep->palet }}</td>
                                    <td class="text-center">{{ $dep->nama_produk }}</td>
                                    <td class="text-center">
                                        {{ Str::isUuid($dep->kode_produksi) ? \App\Models\Mincing::where('uuid', $dep->kode_produksi)->value('kode_produksi') ?? $dep->kode_produksi : $dep->kode_produksi }}
                                    </td>
                                    <td class="text-center">{{ \Carbon\Carbon::parse($dep->exp_date)->format('d-m-Y') }}
                                    </td>
                                    ...
                                    <td class="text-center align-middle">
                                        ...
                                        @can('can access edit button')
                                            <a href="{{ route('sampling_fg.edit.form', $dep->uuid) }}"
                                        ...
                                        @can('can access update button')
                                            <a href="{{ route('sampling_fg.update.form', $dep->uuid) }}"
                                        ...
                                        @can('can access delete button')
                                            <form action="{{ route('sampling_fg.destroy', $dep->uuid) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        <div class="modal fade" id="verifyModal{{ $dep->uuid }}" tabindex="-1"
                                            aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered">
                                                <form action="{{ route('sampling_fg.verification.update', $dep->uuid) }}"
                                                    method="POST">
                                                    @csrf @method('PUT')
                            ...
                            @empty
                                <tr>
                                    <td colspan="21" class="text-center py-3">Belum ada data.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
