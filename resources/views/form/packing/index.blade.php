...
@section('content')
    <div class="container-fluid py-0">
            ...
            <div class="btn-group" role="group">
                ...
                @can('can access export')
                    <a href="{{ route('packing.exportPdf', ['date' => request('date'), 'shift' => request('shift'), 'nama_produk' => request('nama_produk')]) }}"
                        id="btnExportPdf" target="_blank" class="btn btn-danger">
        ...
        <form id="filterForm" method="GET" action="{{ route('packing.index') }}"
            class="d-flex flex-wrap align-items-center gap-5 MB-3 p-3 border rounded bg-white shadow-sm">
            <div class="row w-100">
                <div class="col-md-3">
                        ...
                        <tbody>
                            ...
                            @forelse ($data as $dep)
                                <tr>
                                    ...
                                    <td class="text-center align-middle">
                                        {{ \Carbon\Carbon::parse($dep->waktu)->format('H:i') }}</td>
                                    <td class="text-center align-middle">
                                        @if ($dep)
                                            <a href="#" data-bs-toggle="modal"
                                                data-bs-target="#packingModal{{ $dep->uuid }}"
                                            ...
                                            <div class="modal fade" id="packingModal{{ $dep->uuid }}" tabindex="-1"
                                                aria-labelledby="packingModalLabel{{ $dep->uuid }}"
                                                aria-hidden="true">
                                                <div class="modal-dialog modal-xl">
                                                    <div class="modal-content text-start">
                                                        <div class="modal-header bg-info text-white">
                                                            <h5 class="modal-title"
                                                                id="packingModalLabel{{ $dep->uuid }}">
                                                        ...
                                                        <div class="modal-body">
                                                            <div class="row">
                                                                <div class="col-md-6">
                                                                    <table class="table table-sm table-bordered">
                                                                        <tr>
                                                                            <th>Waktu</th>
                                                                            <td>{{ \Carbon\Carbon::parse($dep->waktu)->format('H:i') }}
                                                                            </td>
                                                                        </tr>
                                                                        <tr>
                                                                ...
                                                                <div class="col-md-6">
                                                                    <table class="table table-sm table-bordered">
                                                                        ...
                                                                        <tr>
                                                                            <th>Berat Pcs</th>
                                                                            <td>{{ $dep->berat_pcs ?? '-' }} gr</td>
                                                                        </tr>
                                                                        <tr>
                                                                            <th>Berat Pack</th>
                                                                            <td>{{ $dep->berat_pack ?? '-' }} gr</td>
                                                                        </tr>
                                                                        <tr>
                                                                            <th>Kondisi Segel</th>
                                                            ...
                                                            <h6 class="mt-4 fw-bold text-primary"><i
                                                                    class="bi bi-box-seam"></i> Data Kemasan:</h6>
                                                            <div class="table-responsive">
                                                                <table
                                                                    ...
                                                                    <tbody>
                                                                        ...
                                                                        @foreach ($kemasans as $item)
                                                                            <tr>
                                                                                <td>{{ $item['jenis_kemasan'] ?? '-' }}
                                                                                </td>
                                                                                <td>{{ $item['no_lot_kemasan'] ?? '-' }}
                                                                                </td>
                                                                                <td>{{ $item['tgl_kedatangan'] ?? '-' }}
                                                                                </td>
                                                                                <td>{{ $item['nama_supplier'] ?? '-' }}
                                                                                </td>
                                                                            </tr>
                                                                        @endforeach
                                                                    </tbody>
                                    ...
                                    <td class="text-center align-middle">
                                        ...
                                        @can('can access edit button')
                                            <a href="{{ route('packing.edit.form', $dep->uuid) }}"
                                        ...
                                        @can('can access delete button')
                                            <form action="{{ route('packing.destroy', $dep->uuid) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        <div class="modal fade" id="verifyModal{{ $dep->uuid }}" tabindex="-1"
                                            ...
                                            <div class="modal-dialog modal-dialog-centered modal-md">
                                                <form action="{{ route('packing.verification.update', $dep->uuid) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                                ...
                                                                <div class="col-md-12">
                                                                    ...
                                                                    <select name="status_spv"
                                                                        ...
                                                                        <option value="1"
                                                                            ...
                                                                            style="color: #198754; font-weight: 600;">âœ…
                                                                            Verified (Disetujui)</option>
                                                                        <option value="2"
                                                                            {{ $dep->status_spv == 2 ? 'selected' : '' }}
                                                                            style="color: #dc3545; font-weight: 600;">âŒ
