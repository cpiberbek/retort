                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="text-center align-middle">
                                ...
                                @if(!empty($hasilSuhu))
                                <a href="javascript:void(0);" class="btn btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#peneraanModal{{ $dep->uuid }}">
                                    Lihat Hasil Pemeriksaan
                                </a>
                                {{-- Modal --}}
                                <div class="modal fade" id="peneraanModal{{ $dep->uuid }}" tabindex="-1" aria-hidden="true">
                                                ...
                                                <table class="table table-bordered table-sm mb-0 text-center align-middle">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            ...
                                                            @foreach($areaList as $area)
                                                            @php
                                                                // LOGIKA BARU PENGECEKAN SUHU
                                                                $matched = collect($hasilSuhu)->firstWhere('area', $area->area);
                                                                $rawNilai = $matched['nilai'] ?? null; // Bisa null, "-", atau angka float
                                                                $standarStr = trim($area->standar ?? '');
                                                                $outOfRange = false;

                                                                // Hanya lakukan pengecekan JIKA datanya berupa angka
                                                                if (is_numeric($rawNilai) && $standarStr !== '') {
                                                                    $nilai = floatval($rawNilai);
                                                                    $standarClean = str_replace(['(', ')'], '', $standarStr);

                                                                    if (preg_match('/^<\s*=?\s*(-?\d+(\.\d+)?)/', $standarStr, $m)) {
                                                                        // Format "< 10"
                                                                        $max = floatval($m[1]);
                                                                        $outOfRange = $nilai > $max; 
                                                                    } else {
                                                                        // Format "0 - 4" atau "(-18) - (-22)"
                                                                        preg_match_all('/-?\d+(\.\d+)?/', $standarClean, $matches);
                                                                ...
                                                                if (!is_numeric($rawNilai)) {
                                                                    $textColor = 'text-dark'; // Jika kosong atau strip, warna standar
                                                                } else {
                                                                    $textColor = $outOfRange ? 'text-danger' : 'text-success';
                                                                }
                                                            @endphp

                                                            <td class="fw-bold text-center {{ $textColor }}">
                                                                {{-- Jika null tampil kosong '', jika ada nilai tampilkan isinya --}}
                                                                {{ $rawNilai ?? 'n/a' }}
                                                            </td>
                                                            @endforeach
                                                        </tr>
                            ...
                            <td class="text-center align-middle">
                                ...
                                {{-- Modal Verification (Disembunyikan sedikit untuk keterbacaan, kodenya tetap sama seperti punya Anda) --}}
                                                ...
                                                <div class="modal-body p-5">
                                                    ...
                                                    <div class="row g-4">
                                                        ...
                                                        <div class="col-md-12 mt-3">
                                                            <label for="catatan_spv_{{ $dep->uuid }}" class="form-label fw-bold text-light mb-2">Catatan Tambahan (Opsional)</label>
