        ...
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
            <div>
                <h4 class="mb-1 fw-bold text-dark">
                    <i class="bi bi-pencil-square text-warning me-2"></i> Edit Pemeriksaan Suhu & RH
                </h4>
                <p class="text-muted small mb-0">Perbarui data form di bawah ini dengan akurat.</p>
            </div>
                    ...
                    <div class="row g-3 g-md-4">
                        <div class="col-12 col-md-4">
                            ...
                            <input type="date" name="date" id="dateInput"
                                class="form-control form-control-solid rounded-3" value="{{ $suhu->date }}" required>
                        </div>
                        <div class="col-12 col-md-4">
                        ...
                        <tbody>
                            @foreach ($area_suhus->filter(function ($area) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" name="hasil_rh[{{ $index }}][nilai]"
                                            value="{{ $nilai_rh }}"
                                            class="form-control rh-input
                                {{ $area->rh_min === null ? 'bg-light text-muted' : '' }}"
                    ...
                    <div class="row g-3 g-md-4">
                        <div class="col-12 col-md-6">
                            ...
                            <textarea name="keterangan" class="form-control form-control-solid rounded-3" rows="3"
                                placeholder="Ketik keterangan di sini...">{{ $suhu->keterangan }}</textarea>
                        </div>
                        <div class="col-12 col-md-6">
                            ...
                            <textarea name="catatan" class="form-control form-control-solid rounded-3" rows="3"
                                placeholder="Ketik catatan di sini...">{{ $suhu->catatan }}</textarea>
                        </div>
                    </div>
                </div>
            </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                ...
                // Fungsi Helper untuk memvalidasi input secara dinamis
                function validateSuhu(input) {
                    ...
                    // Lanjut ke validasi Min/Max dari Dev
                ...
                suhuInputs.forEach(input => {
                    // 1. Eksekusi saat halaman pertama kali di-load (Untuk mengecek data lama)
                    // Lakukan filter awal pada data lama
                    let rawValueInit = input.value.replace(',', '.').trim();
                    if (rawValueInit !== '' && rawValueInit !== '-') {
                        validateSuhu(input);
                    }

                    // 2. Eksekusi saat user mengubah/mengetik nilai baru
            ...
            document.querySelectorAll('.rh-input').forEach(input => {
                input.addEventListener('input', function() {
                    let raw = this.value.replace(',', '.').trim();

                    let val = parseFloat(raw);
                    let min = parseFloat(this.dataset.min);
                    let max = parseFloat(this.dataset.max);
                    let warn = this.parentElement.querySelector('.warning-msg');
