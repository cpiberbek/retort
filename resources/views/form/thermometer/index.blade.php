...
@section('content')
    <div class="container-fluid py-0">
        ...
        <div class="d-sm-flex justify-content-between align-items-center mb-4">
            ...
            <div class="btn-group" role="group">
                ...
                @can('can access export')
                    <a href="{{ route('thermometer.exportPdf', ['date' => request('date')]) }}" target="_blank"
                        id="exportPdfBtn" class="btn btn-danger">
        ...
        <form id="filterForm" method="GET" action="{{ route('thermometer.index') }}"
                        ...
                        <tbody>
                            ...
                            @forelse ($data as $dep)
                                <tr>
                                    ...
                                    <td class="text-center align-middle">
                                        @php
                                            $peneraan = is_array($dep->peneraan)
                                                ? $dep->peneraan
                                                : json_decode($dep->peneraan, true);
                                        @endphp
                                        ...
                                        @if (!empty($peneraan))
                                            ...
                                            <div class="modal fade" id="peneraanModal{{ $dep->uuid }}" tabindex="-1"
                                                aria-labelledby="peneraanModalLabel{{ $dep->uuid }}"
                                                aria-hidden="true">
                                                <div
                                                    ...
                                                    <div class="modal-content">
                                                        <div class="modal-header bg-primary text-white">
                                                            <h5 class="modal-title"
                                                                id="peneraanModalLabel{{ $dep->uuid }}">
                                                                Tanggal :
                                                                ...
                                                                <table
                                                                    ...
                                                                    <tbody>
                                                                        @foreach ($peneraan as $index => $item)
                                                                            <tr>
                                                                                <td>{{ $index + 1 }}</td>
                                                                                <td>{{ $item['kode_thermometer'] ?? '-' }}
                                                                                </td>
                                                                                <td>{{ $item['area'] ?? '-' }}</td>
                                                                                <td>{{ $item['standar'] ?? '-' }}</td>
                                                                                <td>{{ $item['pukul'] ?? '-' }}</td>
                                                                                <td>{{ $item['hasil_tera'] ?? '-' }}</td>
                                                                                <td>{{ $item['tindakan_perbaikan'] ?? '-' }}
                                                                                </td>
                                                                            </tr>
                                                                        @endforeach
                                                                    </tbody>
                                    ...
                                    <td class="text-center align-middle">
                                        ...
                                        @can('can access edit button')
                                            <a href="{{ route('thermometer.edit.form', $dep->uuid) }}"
                                                class="btn btn-warning btn-sm me-1 mb-1">
                                                <i class="bi bi-pencil-square"></i> Edit Data
                                            </a>
                                        @endcan
                                        @can('can access update button')
                                            <a href="{{ route('thermometer.update.form', $dep->uuid) }}"
                                                class="btn btn-info btn-sm me-1 mb-1">
                                                <i class="bi bi-pencil"></i> Update
                                            </a>
                                        @endcan
                                        @can('can access delete button')
                                            <form action="{{ route('thermometer.destroy', $dep->uuid) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        <div class="modal fade" id="verifyModal{{ $dep->uuid }}" tabindex="-1"
                                            ...
                                            <div class="modal-dialog modal-dialog-centered modal-md">
                                                <form action="{{ route('thermometer.verification.update', $dep->uuid) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
