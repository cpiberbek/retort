...
@section('content')
    ...
    <div class="container-fluid py-0">
        ...
        <form id="filterForm" method="GET" action="{{ route('sampling.index') }}"
                    ...
                    <div class="input-group mb-2">
                        ...
                        <select name="nama_produk" id="filter_nama_produk" class="form-select border-start-0 form-control">
                            ...
                            @foreach (\App\Models\Produk::where('plant', Auth::user()->plant)->pluck('nama_produk')->unique() as $produk)
                                <option value="{{ $produk }}"
                                    {{ request('nama_produk') == $produk ? 'selected' : '' }}>{{ $produk }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                        ...
                        <tbody>
                            ...
                            @forelse ($data as $dep)
                                <tr>
                                    ...
                                    <td class="text-center align-middle">{{ formatAngka($dep->pin_hole) }}</td>
                                    <td class="text-center align-middle">{{ formatAngka($dep->air_trap_pvdc) }}</td>
                                    <td class="text-center align-middle">{{ formatAngka($dep->air_trap_produk) }}</td>
                                    <td class="text-center align-middle">{{ formatAngka($dep->keriput) }}</td>
                                    <td class="text-center align-middle">{{ formatAngka($dep->bengkok) }}</td>
                                    <td class="text-center align-middle">{{ formatAngka($dep->non_kode) }}</td>
                                    <td class="text-center align-middle">{{ formatAngka($dep->over_lap) }}</td>
                                    <td class="text-center align-middle">{{ formatAngka($dep->kecil) }}</td>
                                    <td class="text-center align-middle">{{ formatAngka($dep->terjepit) }}</td>
                                    <td class="text-center align-middle">{{ formatAngka($dep->double_klip) }}</td>
                                    <td class="text-center align-middle">{{ formatAngka($dep->seal_halus) }}</td>
                                    <td class="text-center align-middle">{{ formatAngka($dep->basah) }}</td>
                                    <td class="text-center align-middle">{{ formatAngka($dep->dll) }}</td>
                                    <td class="text-center align-middle">
                                        @if ($dep->catatan)
                                            <a href="#" class="text-primary text-decoration-underline"
                                                data-bs-toggle="modal" data-bs-target="#catatanModal{{ $dep->id }}">
                                                Lihat Catatan
                                            </a>

                                            <!-- Modal -->
                                            <div class="modal fade" id="catatanModal{{ $dep->id }}" tabindex="-1"
                                                aria-labelledby="catatanLabel{{ $dep->id }}" aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content">
                                                        <div class="modal-header bg-primary text-white">
                                                            <h5 class="modal-title" id="catatanLabel{{ $dep->id }}">
                                                                Catatan</h5>
                                    ...
                                    <td class="text-center align-middle">
                                        ...
                                        @elseif ($dep->status_spv == 2)
                                            ...
                                            <div class="modal fade" id="revisionModal{{ $dep->uuid }}" tabindex="-1"
                                                aria-labelledby="revisionModalLabel{{ $dep->uuid }}"
                                                aria-hidden="true">
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content">
                                    ...
                                    <td class="text-center align-middle">
                                        ...
                                        @can('can access edit button')
                                            <a href="{{ route('sampling.edit.form', $dep->uuid) }}"
                                                class="btn btn-warning btn-sm me-1 mb-1">
                                                <i class="bi bi-pencil-square"></i> Edit Data
                                            </a>
                                        @endcan
                                        @can('can access update button')
                                            <a href="{{ route('sampling.update.form', $dep->uuid) }}"
                                                class="btn btn-info btn-sm me-1 mb-1">
                                                <i class="bi bi-pencil"></i> Update
                                            </a>
                                        @endcan
                                        @can('can access delete button')
                                            <form action="{{ route('sampling.destroy', $dep->uuid) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        <div class="modal fade" id="verifyModal{{ $dep->uuid }}" tabindex="-1"
                                            ...
                                            <div class="modal-dialog modal-dialog-centered modal-md">
                                                <form action="{{ route('sampling.verification.update', $dep->uuid) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
