...
@section('content')
    <div class="container-fluid py-4 px-3">
        ...
        {{-- Filter Tanggal --}}
        <form id="filterForm" method="GET" action="{{ route('gmp.index') }}"
                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            @forelse($data as $dep)
                                <tr>
                                    ...
                                    <td class="align-middle py-2">
                                        @php
                                            $pemeriksaan = is_array($dep->pemeriksaan)
                                                ? $dep->pemeriksaan
                                                : (json_decode($dep->pemeriksaan, true) ?:
                                                []);
                                        ...
                                        <div class="d-flex flex-wrap gap-1">
                                            @foreach ($groupedByArea as $areaName => $rows)
                                                @php
                                                    ...
                                                    foreach ($rows as $row) {
                                                        $attrKeys = array_diff(array_keys($row), [
                                                            'nama_karyawan',
                                                            'pukul',
                                                            'keterangan',
                                                            'area',
                                                ...
                                                <span class="area-pill {{ $pillClass }}" data-bs-toggle="tooltip"
                                                        ...
                                                        <div class="modal-body text-start text-dark">
                                                            <ul class="list-unstyled mb-0">
                                                                <li><strong>Status:</strong> <span
                                                                        class="text-danger">Revision</span></li>
                                                                <li class="mt-2"><strong>Catatan SPV:</strong><br>
                                                                    {{ $dep->catatan_spv ?? '-' }}</li>
                                                            </ul>
                                                        </div>
                                    ...
                                    <td class="text-center align-middle">
                                        ...
                                        <div class="d-flex flex-wrap justify-content-center">
                                            @can('can access verification button')
                                                <button type="button" class="btn btn-primary btn-sm fw-bold shadow-sm m-1"
                                                    data-bs-toggle="modal" data-bs-target="#verifyModal{{ $dep->uuid }}">
                                                    <i class="bi bi-shield-check"></i> Verifikasi
                                                </button>
                                            @endcan
                                            @can('can access edit button')
                                                <a href="{{ route('gmp.edit.form', $dep->uuid) }}"
                                                    class="btn btn-warning btn-sm m-1 text-dark fw-bold">
                                                    <i class="bi bi-pencil-square"></i> Edit
                                                </a>
                                            @endcan
                                            @can('can access update button')
                                                <a href="{{ route('gmp.update.form', $dep->uuid) }}"
                                                    class="btn btn-info btn-sm m-1 text-white fw-bold">
                                                    <i class="bi bi-pencil"></i> Update
                                                </a>
                                            @endcan
                                            @can('can access delete button')
                                                <form action="{{ route('gmp.destroy', $dep->uuid) }}" method="POST"
                                                    class="d-inline m-1">
                                                    @csrf
                                                    @method('DELETE')
                                        ...
                                        <div class="modal fade" id="verifyModal{{ $dep->uuid }}" tabindex="-1">
                                            <div class="modal-dialog modal-dialog-centered modal-md">
                                                <form action="{{ route('gmp.updateVerification', $dep->uuid) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    ...
                                                    <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden text-white"
                                                        ...
                                                        <div class="modal-body text-start text-dark">
                                                            <label class="form-label fw-bold text-white">Status
                                                                Verifikasi</label>
                                                            <select name="status_spv" class="form-select" required>
                                                                <option value="1">Verified</option>
                                                                <option value="2">Revision</option>
                                                            </select>
                                                            <label class="form-label mt-3 fw-bold text-white">Catatan
                                                                SPV</label>
                                                        ...
                                                        <div class="modal-footer">
                                                            <button type="submit" class="btn btn-success"><i
                                                                    class="bi bi-check-lg"></i> Submit</button>
                                                        </div>
                                                    </div>
                                                </form>
                                            </div>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center py-4 text-muted">Belum ada data GMP karyawan
                                        pada list ini.</td>
                                </tr>
                            @endforelse
                        </tbody>
