    ...
    <div class="card shadow-sm">
        <div class="card-body">
            ...
            <h4 class="mb-3">
                ...
                Update Pemeriksaan Personal Hygiene dan Kesehatan Karyawan (QC)
            </h4>
            ...
            <div class="alert alert-warning mb-4">
                ...
                <ul class="mb-0 mt-2">
                    ...
                    <li>Checkbox yang sudah <b>tercentang (filled)</b> terkunci dan tidak dapat diubah.</li>
                </ul>
            </div>

            <form id="updateForm" method="POST" action="{{ route('gmp.update_qc', $gmp->uuid) }}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="card-body">
                        ...
                        <div class="alert alert-danger py-2 px-3 mb-3" style="font-size: .9rem">
                            ...
                            <ul class="mb-0">
                                ...
                                <li>Checkbox yang sudah <b>tercentang dari data sebelumnya terkunci</b> (tidak dapat diubah).</li>
                            </ul>
                        </div>
                                                            ...
                                                            <tr>
                                                                ...
                                                                @foreach (['anting','kalung','cincin','jam_tangan','peniti','bros','payet','softlens','eyelashes','seragam','boot','masker','ciput_hairnet','kuku','parfum','make_up','diare','demam','luka_bakar','batuk','radang','influenza','sakit_mata'] as $attr)
                                                                    <td class="text-center">
                                                                        ...
                                                                        @endphp
                                                                        @if ($isFilled)
                                                                            {{-- Sudah tercentang sebelumnya: tampilkan terkunci --}}
                                                                            <input type="checkbox" checked disabled onclick="return false;" title="Sudah diisi sebelumnya">
                                                                            <input type="hidden" name="{{ $slugArea }}[{{ $x }}][{{ $attr }}]" value="1">
                                                                        @else
                                                                            {{-- Belum tercentang: bisa diinput baru --}}
                                                                            <input type="hidden" name="{{ $slugArea }}[{{ $x }}][{{ $attr }}]" value="0">
                                                                            <input type="checkbox"
...
<style>
    ...
    /* Checkbox disabled style agar terlihat berbeda */
