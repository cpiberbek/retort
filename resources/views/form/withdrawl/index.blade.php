    ...
    <div class="container-fluid py-0">
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                ...
                <form id="filterForm" method="GET" action="{{ route('withdrawl.index') }}"
                    ...
                    <table class="table table-striped table-bordered align-middle table-hover">
                        ...
                        <tbody>
                            ...
                            @forelse ($data as $dep)
                                <tr>
                                    ...
                                    <td class="text-center align-middle">
                                        {{ \Carbon\Carbon::parse($dep->date)->format('d-m-Y') }} | {{ $dep->no_withdrawl }}
                                    </td>
                                    <td class="text-center align-middle">{{ $dep->nama_produk }}</td>
                                    <td class="text-center align-middle">{{ $dep->kode_produksi }} |
                                    ...
                                    <td class="text-center align-middle">
                                        ...
                                        @if (!empty($withdrawl))
                                            <a href="#" data-bs-toggle="modal"
                                                data-bs-target="#withdrawlModal{{ $dep->uuid }}"
                                                style="font-weight: bold; text-decoration: underline;">
                                                Detail
                                            </a>

                                            <div class="modal fade" id="withdrawlModal{{ $dep->uuid }}" tabindex="-1"
                                                aria-labelledby="withdrawlModalLabel{{ $dep->uuid }}"
                                                aria-hidden="true">
                                                <div class="modal-dialog" style="max-width: 60%;">
                                                    <div class="modal-content">
                                                        <div class="modal-header bg-warning text-white">
                                                            <h5 class="modal-title"
                                                                id="withdrawlModalLabel{{ $dep->uuid }}">
                                    ...
                                    <td class="text-center align-middle">
                                        ...
                                        @else
                                            <b><a href="#"
                                                    class="{{ $dep->status_spv == 1 ? 'text-success' : 'text-danger' }} fw-bold"
                                        ...
                                        <div class="modal fade" id="modalSpv{{ $dep->uuid }}" tabindex="-1">
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content">
                                                    <div
                                                        class="modal-header {{ $dep->status_spv == 1 ? 'bg-success' : 'bg-danger' }} text-white">
                                                        <h5 class="modal-title">
                                                    ...
                                                    <div class="modal-body">
                                                        <ul class="list-unstyled">
                                                            <li><strong>Status:</strong>
                                                                {{ $dep->status_spv == 1 ? 'Verified' : 'Revision' }}</li>
                                                            <li><strong>Supervisor:</strong> {{ $dep->nama_spv ?? '-' }}
                                                            </li>
                                                            <li><strong>Catatan:</strong> {{ $dep->catatan_spv ?? '-' }}
                                                            </li>
                                                        </ul>
                                                    </div>
                                                </div>
                                    ...
                                    <td class="text-center align-middle">
                                        ...
                                        @else
                                            <b><a href="#"
                                                    class="{{ $dep->status_manager == 1 ? 'text-success' : 'text-danger' }} fw-bold"
                                                    data-bs-toggle="modal"
                                                    data-bs-target="#modalManager{{ $dep->uuid }}">
                                        ...
                                        <div class="modal fade" id="modalManager{{ $dep->uuid }}" tabindex="-1">
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content">
                                                    <div
                                                        class="modal-header {{ $dep->status_manager == 1 ? 'bg-success' : 'bg-danger' }} text-white">
                                                        <h5 class="modal-title">
                                                    ...
                                                    <div class="modal-body">
                                                        <ul class="list-unstyled">
                                                            <li><strong>Status:</strong>
                                                                {{ $dep->status_manager == 1 ? 'Verified' : 'Revision' }}
                                                            </li>
                                                            <li><strong>Manager:</strong> {{ $dep->nama_manager ?? '-' }}
                                                            </li>
                                                            <li><strong>Catatan:</strong>
                                                                {{ $dep->catatan_manager ?? '-' }}</li>
                                                        </ul>
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    <td class="text-center align-middle">
                                        ...
                                        @can('can access verification manager')
                                            <button type="button" class="btn btn-info btn-sm fw-bold shadow-sm"
                                                data-bs-toggle="modal"
                                                data-bs-target="#verifyManagerModal{{ $dep->uuid }}">
                                                <i class="bi bi-person-check-fill me-1"></i> Persetujuan Manager
                                            </button>
                                        @endcan
                                        @can('can access edit button')
                                            <a href="{{ route('withdrawl.edit', $dep->uuid) }}"
                                                class="btn btn-warning btn-sm">
                                                <i class="bi bi-pencil"></i> Edit
                                            </a>
                                        @endcan
                                        @can('can access delete button')
                                            <form action="{{ route('withdrawl.destroy', $dep->uuid) }}" method="POST"
                                                class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                        ...
                                        <div class="modal fade" id="verifyModal{{ $dep->uuid }}" tabindex="-1"
                                            ...
                                            <div class="modal-dialog modal-dialog-centered modal-md">
                                                <form action="{{ route('withdrawl.updateVerification', $dep->uuid) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    ...
                                                    <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden text-white"
                                                        ...
                                                        <div class="modal-body p-5">
                                                            <p class="text-light mb-4 fs-6">Pastikan data yang akan
                                                                diverifikasi dicek dengan teliti terlebih dahulu.</p>

                                                            <label class="form-label fw-bold mb-2 text-center d-block"
                                                                style="color: #FFE5DE;">Status Verifikasi SPV</label>
                                                            ...
                                                            <br>
                                                            <label class="form-label fw-bold text-light mt-3">Catatan SPV
                                                                (Opsional)
                                                            </label>
                                                            <textarea name="catatan_spv" rows="4" class="form-control text-dark border-0"
                                                                style="background-color: #FFE5DE;">
                            {{ $dep->catatan_spv }}</textarea>
                                        ...
                                        <div class="modal fade" id="verifyManagerModal{{ $dep->uuid }}"
                                            tabindex="-1" aria-labelledby="verifyManagerModalLabel{{ $dep->uuid }}"
                                            aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered modal-md">
                                                <form action="{{ route('withdrawl.updateApproval', $dep->uuid) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('PUT')
                                                    <input type="hidden" name="page"
                                                    ...
                                                    <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden text-white"
                                                        ...
                                                        <div class="modal-header p-4 border-bottom border-light">
                                                            <h5 class="modal-title fw-bolder fs-3 text-uppercase"
                                                                style="color: #00eaff;">
                                                                <i class="bi bi-person-check-fill me-2"></i> MANAGER
                                                                APPROVAL
                                                            </h5>
                                                            <button type="button" class="btn-close btn-close-white"
                                                                data-bs-dismiss="modal"></button>
                                                        ...
                                                        <div class="modal-body p-5">
                                                            <p class="text-light mb-4 fs-6">Pastikan data telah ditinjau
                                                                sebelum memberikan persetujuan.</p>

                                                            <label class="form-label fw-bold mb-2 text-light">Status
                                                                Persetujuan Manager</label>
                                                            <br>
                                                            <select name="status_manager"
                                                                ...
                                                                <option value="1"
                                                                    {{ $dep->status_manager == 1 ? 'selected' : '' }}>✔️
                                                                    Approved</option>
                                                                <option value="2"
                                                                    {{ $dep->status_manager == 2 ? 'selected' : '' }}>❌
                                                                    Revision Required</option>
                                                            </select>
                                                            <br>
                                                            <label class="form-label fw-bold text-light mt-3">Catatan
                                                                Manager (Opsional)</label>
                                                            <textarea name="catatan_manager" rows="4" class="form-control text-dark" style="background-color: #d1ecff;">{{ $dep->catatan_manager }}</textarea>
                                                        </div>

                                                        <div class="modal-footer justify-content-end p-4"
                                                            style="background-color: #1e5f92;">
                                                            <button type="button"
                                                                class="btn btn-outline-light fw-bold rounded-pill px-4 me-2"
                                                                data-bs-dismiss="modal">Batal</button>
                                                            <button type="submit" class="btn fw-bolder rounded-pill px-5"
                                                                style="background-color: #00eaff; color: #003449;">
                                                                <i class="bi bi-save-fill me-1"></i> SUBMIT
                                                            </button>
                                                        </div>
